...
이 토큰은 EKS가 요청자를 AWS IAM principal로 검증할 수 있게 해주는 증빙입니다.
...
- `CONFIG_MAP`: 레거시 `aws-auth` 중심 (현재는 사용이 불가합니다.)
...
이 ConfigMap은 다음 역할을 했습니다.

- 어떤 IAM principal을 받을지 정의
- 그 principal을 어떤 Kubernetes group으로 매핑할지 정의
...
Pod가 떠 있는 Worker Node에 부여된 IAM 역할을 IMDS를 통해 수임하는 방식입니다. `aws sts assume-role`을 통해 만료 시간이 있는 임시 보안 자격 증명을 생성하므로 하드코딩된 액세스키보다는 보안 위험이 낮습니다. 하지만 아래 이유로 여전히 권장되지 않는 방식입니다.
...
`aud` 필드가 다릅니다. IRSA 토큰은 **AWS STS 전용**으로 발급됩니다. 아래는 실제 IRSA 토큰 페이로드입니다.
...
IRSA가 기술적으로 좋은 방식인 것과, 운영이 단순하다는 것은 다른 이야기입니다.

- **OIDC Provider 관리**: 클러스터마다 IAM에 OIDC Provider를 등록해야 한다. **AWS 계정당 OIDC Provider 기본 한도는 100개**다. 멀티클러스터 환경에서 이 한도에 금방 도달할 수 있다.
- **Trust policy 복잡도**: 클러스터, namespace, ServiceAccount 수가 늘어날수록 trust policy의 Condition 블록이 길어지고 관리 포인트가 늘어난다. 신뢰 정책 크기 한도로 단일 trust policy에는 최대 8개의 신뢰 관계만 정의할 수 있다.
...
IRSA와 결정적인 차이: Pod가 직접 STS를 호출하지 않습니다. 노드의 Agent를 경유하고, Agent가 EKS Auth API와 통신합니다. OIDC 서명 검증 절차가 없습니다.
...
IRSA는 Pod가 직접 STS를 호출하는 구조이고, Pod Identity는 Agent에 credential 요청을 위임하는 구조라는 차이가 환경변수에서도 드러납니다.
...
IRSA와 비교하면 단순합니다. OIDC Provider ARN도 없고, `sub` 조건도 없습니다. "어떤 Pod가 이 role을 쓸 수 있는가"는 trust policy가 아니라 **EKS association**이 결정합니다.

`sts:TagSession`이 포함된 이유는 Pod Identity가 세션 태그를 지원하기 때문입니다. 임시 자격증명에 클러스터 이름, namespace, ServiceAccount 이름이 태그로 붙어 CloudTrail 분석과 ABAC 기반 정책 설계에 활용할 수 있습니다.
...
Pod Identity는 `AssumeRoleForPodIdentity` 이벤트가 기록됩니다.
...
IRSA의 `sts.amazonaws.com`과 달리 `eks-auth.amazonaws.com`에서 이벤트가 발생합니다. 트러블슈팅 시 어떤 방식을 쓰는지에 따라 확인해야 할 CloudTrail 이벤트가 다릅니다.
...
- EKS 외 환경(EKS Anywhere, 자체 관리 K8s)에서도 같은 패턴을 쓰고 싶은 경우
- 기존 IRSA 운영 체계가 이미 안정적으로 자리 잡은 경우
- OIDC trust policy 관리가 조직 내 표준으로 정착된 경우
- Windows 노드 또는 Fargate 워크로드를 운영하는 경우

**EKS Pod Identity가 적합한 경우**

- 신규 EKS 플랫폼을 설계하는 경우
- 멀티클러스터 환경에서 OIDC Provider 관리 부담을 줄이고 싶은 경우
- 동일한 IAM role을 여러 클러스터에서 재사용하고 싶은 경우
- EKS addon(AWS LBC, ExternalDNS, EBS CSI 등)의 IAM 권한을 EKS API 중심으로 관리하고 싶은 경우

프로덕션 환경에서는 아래와 같이 운영하는 것이 안정적입니다.

- 이미 사용중인 IRSA는 굳이 걷어내지 않습니다.
- **신규 워크로드는 Pod Identity를 우선 검토**합니다.
- Node Role은 사용을 최소화하고, 만일 사용 시에는 IMDS hop limit 설정(IMDSv2 + hop limit 1)으로 노드 자격증명 노출을 제한합니다.
...
예를 들어 특정 namespace 읽기 권한만 주고 싶다면 다음과 같이 설계할 수 있습니다.
...
이 구조에서 Access Entry는 IAM role을 `eks:team-a-readonly` 같은 group으로 연결하고, 실제 권한은 RBAC이 해석합니다.
