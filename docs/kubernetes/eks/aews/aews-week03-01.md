...
1. 전체 구조 (Terraform 리소스)
CPA가 작동하려면 다음 4가지 리소스가 필요합니다.
...
2. 테라폼 코드 예시
```Terraform
# 1. 스케일링 정책 설정 (ConfigMap)
resource "kubernetes_config_map" "coredns_cpa_config" {
  metadata {
...
# 2. RBAC 설정 (권한 부여)
resource "kubernetes_service_account" "cpa_sa" {
  metadata {
    name      = "coredns-autoscaler"
    namespace = "kube-system"
  }
...
resource "kubernetes_cluster_role" "cpa_role" {
  metadata {
    name = "coredns-autoscaler-role"
  }
  ...
  # ConfigMap을 업데이트하여 상태를 기록하는 경우 필요
  rule {
    api_groups = [""]
...
resource "kubernetes_cluster_role_binding" "cpa_role_binding" {
  metadata {
    name = "coredns-autoscaler-binding"
  }
  role_ref {
    api_group = "rbac.authorization.k8s.io"
    kind      = "ClusterRole"
    name      = kubernetes_cluster_role.cpa_role.metadata[0].name
  }
  subject {
    kind      = "ServiceAccount"
    name      = kubernetes_service_account.cpa_sa.metadata[0].name
    namespace = "kube-system"
  }
}

# 3. CPA 배포 (Deployment)
resource "kubernetes_deployment" "coredns_cpa" {
  metadata {
    name      = "coredns-autoscaler"
    namespace = "kube-system"
  }
  ...
  spec {
    ...
    template {
      ...
      spec {
        service_account_name = kubernetes_service_account.cpa_sa.metadata[0].name
        container {
          name  = "autoscaler"
          ...
          args = [
            "--namespace=kube-system",
            "--configmap=${kubernetes_config_map.coredns_cpa_config.metadata[0].name}",
